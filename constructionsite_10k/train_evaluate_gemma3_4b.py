"""
Gemma 3 4B LoRA 微调 + 测试评估（基础模型 vs 微调模型）
...
OUTPUT_DIR        = "/root/autodl-tmp/outputs_gemma3"
LORA_DIR          = "/root/autodl-tmp/gemma3_4b_lora"
EVAL_OUTPUT       = "/root/autodl-tmp/eval_results_gemma3.json"
...
MAX_TEST_SAMPLES  = None    # 调试时改成 50
EVAL_BATCH_SIZE = 8
# ==================== 优化后的 System Prompt ====================
...
def evaluate_model(model, processor, test_data, image_root, desc="评估中", eval_batch_size=8):
    """批量推理评估，eval_batch_size 控制每次推理几条"""
    from rouge_score import rouge_scorer
    from sentence_transformers import SentenceTransformer, util

    rouge  = rouge_scorer.RougeScorer(["rougeL"], use_stemmer=True)
    sbert  = SentenceTransformer("/root/autodl-tmp/all-MiniLM-L6-v2")
    ...
    for batch_start in tqdm(range(0, len(prepared), eval_batch_size), desc=desc):
        ...
        # 批量 tokenize（Gemma3 processor 支持 batch）
...
class ConstructionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        prompt_inputs = self.processor(text=prompt_only, images=image, return_tensors="pt")
...
print(f"峰值显存: {torch.cuda.max_memory_reserved()/1024**3:.2f} GB")
